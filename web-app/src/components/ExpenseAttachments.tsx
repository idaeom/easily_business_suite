...
import { uploadExpenseAttachment, deleteExpenseAttachment } from "@/app/actions";
import { Button } from "@/components/ui/button";
import { FileIcon, Trash2, Upload, Eye, FileText, Image as ImageIcon } from "lucide-react";
import { formatDistanceToNow } from "date-fns";
import Link from "next/link";

export function ExpenseAttachments({ expenseId, attachments, isDisbursed }: { expenseId: string, attachments: any[], isDisbursed: boolean }) {
    ...
    const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            await uploadExpenseAttachment(expenseId, formData);
        } catch (error) {
